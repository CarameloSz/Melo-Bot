...
require('http').createServer((req, res) => res.end(` Me = Mina Myoui Protector OwO`)).listen(3000) //Dont remove this 
...
client.on('interactionCreate', async interaction => {
    ...
    if(funny === 'first') {
        ...
        .setDescription("● To get help on a specific command type `@Melo Music help <command>`!")
        .setColor("#303037")
        .setFooter("Melo Music", ee.avatara)
        .setTimestamp()
...
client.on('interactionCreate', async interaction => {
    ...
    if(player === undefined) {
        if(interaction.channel.name === 'Melo-requets') {
            setTimeout(() => {
...
client.on('guildCreate', async (guild) => {
  ...
  try {
    ...
    const texts = "Hello, Its Me, Melo Music Bot, I Want to Say  Thank You so much For inviting Me i wish You luck!"
    const guildembed = new Discord.MessageEmbed()
    .setTitle("Thank you for adding me in your server!")
    .setColor("#303037")
...
client.on('guildDelete', async (guild) => {
    try {
      ...
      client.channels.cache.get("903815481964761160").send({ embeds: [embed] })
    } catch (e) { console.log(e) }
});
